...
class SubjectDetailScreen extends StatefulWidget {
  ...
  /// Inclusive calendar range every figure and every history row on this page is
  /// confined to, or null for the whole of the subject's history.
  ///
  /// Only meaningful alongside [reportLabel]. Null with a label set is the
  /// report's own Semester mode, which counts every record the semester holds —
  /// see [SubjectReportArgs].
  final DateTimeRange? reportRange;

  /// Set when this page was opened from the Analytics & Reports breakdown, and
  /// names the period the report covers ('Semester 3', 'Mar 10 – Mar 20, 2020').
  ///
  /// This is what makes the page a *report* rather than the live view: the
  /// figures describe a closed period, so the "Safe to Skip" projection — which
...
/// What Analytics & Reports hands to [SubjectDetailScreen] through the router's
/// `extra` when one of its subject-breakdown cards is opened.
///
/// A record rather than a class for the same reason [AttendanceUpsert] is one:
/// it is a call argument bundle with no behaviour of its own.
///
/// `range` is null for a Semester report and carries the picked span for a
/// Custom Dates one — mirroring exactly how the report itself counts each mode
/// (`getAttendanceStats` for the whole semester, `getAttendanceStatsForDateRange`
/// for a span), so the page's numbers can never disagree with the card that
/// opened it. `label` is how the report described that period.
typedef SubjectReportArgs = ({
  Subject subject,
  DateTimeRange? range,
  String label,
});
...
class _SubjectDetailScreenState extends State<SubjectDetailScreen>
  ...
  /// are range-scoped, and the "Safe to Skip" projection is withheld.
  bool get _isReportScope => widget.reportLabel != null;

  /// The scoped range as `yyyy-MM-dd` bounds for the DAO, or null when the page
  /// covers the subject's whole history.
  ({String start, String end})? get _rangeKeys {
    final range = _isReportScope ? widget.reportRange : null;
    if (range == null) return null;
    final fmt = DateFormat('yyyy-MM-dd');
    return (start: fmt.format(range.start), end: fmt.format(range.end));
  }
  ...
  Future<void> _loadHistory() async {
    ...
    final records = keys == null
        ? await _attendanceDao.getAttendanceHistoryForSubject(
            widget.subject.id!,
          )
        : await _attendanceDao.getAttendanceHistoryForSubjectInRange(
            widget.subject.id!,
            keys.start,
            keys.end,
          );
    ...
    // Calculate skippable dates from this subject's own lecture history. Only
            ...
            : Center(
                child: ConstrainedBox(
                  ...
                  child: CustomScrollView(
                    // The whole page is one scroll surface. The stats card, skip
                    // calculator and section header used to sit in a Column above
                    // an Expanded ListView, so they were pinned in place and only
                    // the history could scroll. As slivers they scroll with the
                    // list while the list itself stays lazily built.
                    slivers: [
                      // OVERALL STATS CARD
                      SliverToBoxAdapter(
                          ...
                          child: Padding(
                            ...
                            child: Container(
                              ...
                              child: Column(
                                children: [
                                  Text(
                                    // A report states a subject's standing over
                                    // a period that has closed; "Current" would
                                    // claim it is where the subject stands now.
                                    _isReportScope
                                  ...
                                  // Names the period every number below is
                                  // counted over, so a percentage that differs
                                  // from the dashboard's reads as scoped rather
                      ...
                      // SKIP CALCULATOR — live view only.
                      //
                      // A report answers "where did this subject stand over
                      // this period"; "you can skip N upcoming lectures" is a
                      // projection past that period's end, so it belongs to the
                      ...
                      if (_history.isEmpty)
                        SliverFillRemaining(
                          // Centres the empty message in the space left below the
                          // cards, the way the old Expanded did.
                          hasScrollBody: false,
                          child: Center(
                            child: Padding(
                              padding: const EdgeInsets.symmetric(
                              ...
                              child: Text(
                                // "yet" is wrong for a closed period — nothing
                                // more is going to arrive in it.
                                _isReportScope
