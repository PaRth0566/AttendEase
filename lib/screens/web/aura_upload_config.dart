...
class AuraUploadConfig extends StatefulWidget {
  final Function(List<Subject> subjects, Map<int, Map<String, int>> stats, Map<String, String> meta, double overallTarget, double subjectTarget)? onConfigured;
...
class _AuraUploadConfigState extends State<AuraUploadConfig> {
  ...
  Future<void> _processFile(String name, List<int> bytes) async {
    try {
      ...
      final request = http.MultipartRequest(
        'POST',
        Uri.parse('https://attendease-backend-ndxs.onrender.com/api/analyze-attendance'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          try {
            ...
            final String cleanedJson = rawJsonInsights.replaceAll('```json', '').replaceAll('```', '').trim();
            ...
            final Map<String, String> meta = {
              ...
              'reportStartDate': (parsedJson['reportStartDate'] ?? '').toString(),
  ...
  Widget build(BuildContext context) {
    if (_parsedSubjects != null && _parsedStats != null && widget.onConfigured == null) {
            ...
            children: [
                ...
                style: TextStyle(
                  ...
                  foreground: Paint()
                    ..shader = LinearGradient(
                      colors: isDark
                          ? [const Color(0xFFA5B4FC), const Color(0xFFC7D2FE), const Color(0xFF818CF8)]
                          : [const Color(0xFF4F46E5), const Color(0xFF7C3AED), const Color(0xFF4338CA)],
              ...
              Text(
                ...
                style: TextStyle(
                  ...
                  height: 1.5,
                  color: isDark ? const Color(0xFFC7D2FE).withOpacity(0.8) : const Color(0xFF475569),
                ),
              ),
              ...
              LayoutBuilder(builder: (context, constraints) {
                ...
                final bannersWidget = LayoutBuilder(builder: (context, innerConstraints) {
                  if (width < 768) {
                     return Column(
                       children: [
                         _buildBanner(isDark, Icons.warning_rounded, 'SVKM/Mithibai Students', 'Download your detailed attendance report directly from the SAP Portal before uploading.', false, isMobile),
                         const SizedBox(height: 12),
                         _buildBanner(isDark, Icons.info_outline_rounded, 'Disclaimer', 'These insights are AI-generated. Always verify predictive models with official institutional records.', true, isMobile),
                       ],
                     );
                  }
                  return IntrinsicHeight(
                    child: Row(
                      ...
                      children: [
                        Expanded(child: _buildBanner(isDark, Icons.warning_rounded, 'SVKM/Mithibai Students', 'Download your detailed attendance report directly from the SAP Portal before uploading.', false, isMobile)),
                        const SizedBox(width: 24),
                        Expanded(child: _buildBanner(isDark, Icons.info_outline_rounded, 'Disclaimer', 'These insights are AI-generated. Always verify predictive models with official institutional records.', true, isMobile)),
                      ],
                    ),
                  );
                });

                final bentoWidget = LayoutBuilder(builder: (context, innerConstraints) {
                  if (width < 900) {
                     return Column(
                       children: [
                  ...
                  return IntrinsicHeight(
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        Expanded(flex: 1, child: _buildSettingsCard(isDark, isMobile)),
                        const SizedBox(width: 32),
                        Expanded(flex: 2, child: _buildUploadCard(isDark, isMobile)),
                      ],
                    ),
                  );
                });
                ...
                const SizedBox(height: 32),
                 Container(
                   ...
                   decoration: BoxDecoration(
                     color: isDark ? const Color(0xFF9F1239).withOpacity(0.2) : const Color(0xFFFFE4E6),
                     border: Border.all(color: isDark ? const Color(0xFFBE123C).withOpacity(0.4) : const Color(0xFFFDA4AF)),
                   ...
                   child: Row(
                     children: [
                       ...
                       Expanded(child: Text(_errorMessage!, style: TextStyle(color: isDark ? const Color(0xFFFDA4AF) : const Color(0xFF9F1239)))),
                     ],
                   ),
                 )
              ],
  ...
  Widget _buildBanner(bool isDark, IconData icon, String title, String desc, bool isInfo, bool isMobile) {
    return ClipRRect(
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
             ...
             border: Border.all(
               color: isInfo 
                 ? (isDark ? const Color(0xFFF43F5E).withOpacity(0.2) : const Color(0xFFFDA4AF))
                 : (isDark ? const Color(0xFFF59E0B).withOpacity(0.2) : const Color(0xFFFDE68A)),
             ),
             boxShadow: [
               BoxShadow(
          ...
          child: Row(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: isInfo ? const Color(0xFFF43F5E).withOpacity(0.1) : const Color(0xFFF59E0B).withOpacity(0.1),
                  shape: BoxShape.circle,
                  border: Border.all(
                    color: isInfo ? const Color(0xFFF43F5E).withOpacity(0.2) : const Color(0xFFF59E0B).withOpacity(0.2),
                  )
                ),
                child: Icon(icon, color: isInfo ? const Color(0xFFFB7185) : const Color(0xFFFBBF24), size: isMobile ? 18 : 24),
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(title, style: TextStyle(fontSize: isMobile ? 13 : 16, fontWeight: FontWeight.w600, color: isDark ? const Color(0xFFEEF2FF) : const Color(0xFF1E293B))),
                    SizedBox(height: isMobile ? 2 : 4),
                    Text(desc, style: TextStyle(fontSize: isMobile ? 11 : 14, color: isDark ? const Color(0xFFC7D2FE).withOpacity(0.7) : const Color(0xFF64748B), height: 1.4)),
                  ],
                ),
              ),
            ],
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: isDark ? const Color(0xFF1E1B4B).withOpacity(0.4) : Colors.white.withOpacity(0.8),
            borderRadius: BorderRadius.circular(isMobile ? 16 : 24),
            border: Border.all(color: isDark ? const Color(0xFF818CF8).withOpacity(0.2) : Colors.black.withOpacity(0.05)),
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Set Targets', style: TextStyle(fontSize: isMobile ? 18 : 28, fontWeight: FontWeight.w600, color: isDark ? const Color(0xFFEEF2FF) : const Color(0xFF1E293B))),
              if (!isMobile) ...[
                const SizedBox(height: 4),
                Text('Define thresholds for early warning alerts.', style: TextStyle(fontSize: 12, color: isDark ? const Color(0xFFC7D2FE).withOpacity(0.7) : const Color(0xFF64748B))),
              ],
              ...
              if (isMobile)
                Row(
                  children: [
                    Expanded(child: _buildTargetField(isDark, 'OVERALL (%)', null, _overallTargetCtrl, isMobile)),
                    const SizedBox(width: 12),
                    Expanded(child: _buildTargetField(isDark, 'SUBJECT (%)', null, _subjectTargetCtrl, isMobile)),
                  ],
                )
              else ...[
                _buildTargetField(isDark, 'OVERALL TARGET (%)', 'Institutional minimum requirement.', _overallTargetCtrl, isMobile),
                const SizedBox(height: 24),
                _buildTargetField(isDark, 'SUBJECT TARGET (%)', 'Minimum per-course threshold.', _subjectTargetCtrl, isMobile),
              ],
            ],
          ),
        ),
  ...
  Widget _buildTargetField(bool isDark, String label, String? subtext, TextEditingController ctrl, bool isMobile) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: TextStyle(fontSize: isMobile ? 10 : 12, fontWeight: FontWeight.w600, letterSpacing: 0.5, color: isDark ? const Color(0xFFA5B4FC).withOpacity(0.8) : const Color(0xFF64748B))),
        ...
        Container(
           ...
           decoration: BoxDecoration(
             color: isDark ? const Color(0xFF0F172A).withOpacity(0.8) : Colors.white,
             borderRadius: BorderRadius.circular(8),
             border: Border.all(color: isDark ? const Color(0xFF6366F1).withOpacity(0.3) : const Color(0xFFE2E8F0)),
             boxShadow: [
               BoxShadow(
           ...
           child: TextField(
             ...
             style: TextStyle(color: isDark ? Colors.white : const Color(0xFF1E293B), fontSize: isMobile ? 14 : 16),
        ...
        if (subtext != null) ...[
          const SizedBox(height: 4),
          Text(subtext, style: TextStyle(fontSize: 12, color: isDark ? const Color(0xFFC7D2FE).withOpacity(0.5) : const Color(0xFF94A3B8))),
        ]
      ],
    );
  }
      ...
      child: ClipRRect(
        ...
        child: BackdropFilter(
          ...
          child: Container(
            ...
            padding: EdgeInsets.symmetric(vertical: isMobile ? 24 : 60, horizontal: isMobile ? 20 : 32),
            decoration: BoxDecoration(
              color: _isDragging 
                 ? (isDark ? const Color(0xFF312E81).withOpacity(0.6) : const Color(0xFFE0E7FF).withOpacity(0.8))
                 : (isDark ? const Color(0xFF1E1B4B).withOpacity(0.4) : Colors.white.withOpacity(0.8)),
              borderRadius: BorderRadius.circular(24),
              border: Border.all(
                 color: _isDragging 
                ...
                if (_isLoading) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      shape: BoxShape.circle,
                      border: Border.all(color: isDark ? const Color(0xFF818CF8).withOpacity(0.3) : const Color(0xFFC7D2FE)),
                      boxShadow: [
                        BoxShadow(
                    ...
                    child: Icon(Icons.cloud_upload_rounded, size: isMobile ? 48 : 72, color: isDark ? const Color(0xFFA5B4FC) : const Color(0xFF6366F1)),
                  ),
                  SizedBox(height: isMobile ? 12 : 24),
                  Text(
                    _fileName ?? 'Drop your PDF report here',
                    style: TextStyle(fontSize: isMobile ? 20 : 24, fontWeight: FontWeight.w600, color: isDark ? const Color(0xFFEEF2FF) : const Color(0xFF1E293B)),
                  ),
                  const SizedBox(height: 12),
                  if (_fileName == null)
                    RichText(
                       text: TextSpan(
                          text: 'or ',
                          style: TextStyle(color: isDark ? const Color(0xFFC7D2FE).withOpacity(0.8) : const Color(0xFF64748B), fontSize: 16),
                          children: [
                             TextSpan(
                               text: 'click to browse',
                               style: TextStyle(color: isDark ? const Color(0xFF818CF8) : const Color(0xFF4F46E5)),
                             ),
                    ...
                    const Text('Click to replace file', style: TextStyle(color: Color(0xFFC7D2FE))),
                ]
              ],
            ),
          ),
